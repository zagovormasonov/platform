...
import { User, Eye, Tag, Clock, Heart, Bookmark, HeartOff } from 'lucide-react'
...
export function LikedPage() {
  ...
  const fetchLikedArticles = async () => {
    try {
      ...
      // Получаем понравившиеся статьи пользователя
      ...
      if (error) {
        ...
        setError('Не удалось загрузить понравившиеся статьи')
        return
      }

      // Получаем избранное пользователя для этих статей
      ...
      if (articleIds.length > 0) {
        ...
        userFavorites = Array.isArray(favoritesData) ? favoritesData.map(fav => fav.article_id) : []
      }
      ...
      const typedData = Array.isArray(data) ? data.map(item => {
        ...
        return {
          ...article,
          is_liked: true, // Все статьи здесь лайкнуты
      ...
      console.error('Ошибка загрузки понравившихся статей:', err)
  ...
  const toggleLike = async (articleId: string) => {
    try {
      // Убираем лайк (удаляем из списка)
        ...
        <div className="min-h-screen bg-gray-50 py-8">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center py-12">
              ...
              <p className="mt-4 text-gray-600">Загрузка понравившихся статей...</p>
            </div>
          </div>
        </div>
      </PageLayout>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-8">
            <div className="flex items-center justify-center mb-4">
              ...
              <h1 className="text-3xl font-bold text-gray-900">Понравившиеся статьи</h1>
            </div>
            <p className="text-gray-600">
              {articles.length === 0 ? 'У вас пока нет понравившихся статей' : `${articles.length} статей вам понравились`}
            </p>
          </div>
          ...
          {articles.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900">Нет понравившихся статей</h3>
              <p className="mt-2 text-gray-600">
                Ставьте лайки статьям, которые вам нравятся
              </p>
            </div>
          ) : (
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                        <button
                          ...
                          className="flex items-center space-x-2 px-3 py-1.5 rounded-lg bg-red-100 text-red-600 hover:bg-red-200 transition-all"
                        >
                          <HeartOff className="h-4 w-4" />
                          <span className="text-sm">Убрать лайк</span>
                        </button>
                        ...
                        >
                          ...
                          <span className="text-sm">{article.is_favorited ? 'В избранном' : 'В избранное'}</span>
                        </button>
                      </div>
                    </div>
                  </article>
